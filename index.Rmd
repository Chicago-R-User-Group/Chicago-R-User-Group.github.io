...
plot(y = seq(1, nrow(member_list)), x = member_list$Joined.Group.on, type = "l", lwd=2, 
...
barplot(new_members$New[Date_Index], names.arg = x_labels, las=2,
       main = "CRUG members, joined between meetups", ylim=c(0,4000),
       ylab = "New Members", xlab = "", col="lightblue")
```

```{r echo = FALSE, fig.width = 12, fig.height = 5}
members_xts <- xts(x = seq(1, nrow(member_list)), order.by = member_list$Joined.Group.on)
names(members_xts) <- "useRs"

members_monthly <- to.monthly(members_xts, OHLC = FALSE)
barplot(diff(members_monthly), col = "cornflowerblue", 
...
barplot(members_3_years, col = "lightblue3", main = "CRUG Growth, last 4 years", ylab="Percent")
```
...
location_agg <- within(location_agg, 
                       hover <- paste(code, '<br>', "Total:", Member.ID)
               )
```
